...
from litestar.params import FromQuery, Parameter, QueryParameter
...
PluginTypeParam = Annotated[
    ...
    QueryParameter(description="The plugin type"),
]


PluginTypeOrAnyParam = Annotated[
    # str,
    api_schemas.PluginTypeOrAnyEnum,  # TODO: Schemathesis doesn't like constraints on path parameters
    QueryParameter(description="The plugin type"),
]
...
class PluginsController(Controller):
    ...
    dependencies: ClassVar[Dependencies] = {  # type: ignore[misc]  # ty: ignore[invalid-attribute-override]
        "hub": Provide(get_hub),
    }
    ...
    async def get_index(self, hub: NamedDependency[client.MeltanoHub]) -> api_schemas.PluginIndex:  # ruff: ignore[no-self-use]
    ...
    async def get_type_index(  # ruff: ignore[no-self-use]
        self,
    ...
    async def get_variants(
        ...
        if match := PLUGIN_VARIANT_PATTERN.match(plugin_name):
            ...
            return Redirect(path=f"{self.path}/{plugin_type.value}/{plugin_name}/{variant}")  # type: ignore[return-value]  # ty: ignore[invalid-return-type]
    ...
    async def get_variant_details(  # ruff: ignore[no-self-use]
        self,
        hub: NamedDependency[client.MeltanoHub],
        plugin_type: PluginTypeParam,
        plugin_name: PluginNameParam,
        variant: PluginVariantParam,
        meltano_version: NamedDependency[compatibility.VersionTuple],
    ) -> (
    ...
    async def sdk(  # ruff: ignore[no-self-use]
        self,
        hub: NamedDependency[client.MeltanoHub],
        *,
        limit: FromQuery[int] = 25,
        plugin_type: PluginTypeOrAnyParam,
    ...
    async def stats(self, hub: NamedDependency[client.MeltanoHub]) -> dict[enums.PluginTypeEnum, int]:  # ruff: ignore[no-self-use]
